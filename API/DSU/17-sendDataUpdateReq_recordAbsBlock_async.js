...
// Encrypted ciphertext of the new version of data to be uploaded.
...
// Data modification request user
var requestor = '0x0000000000000000000000000000000000000000'
...
// DSU sends data sharing requests and records write summary blocks
let sendDataUpdateReq_recordAbsBlock = async (myJSON) => {
	try {
		let content = myJSON + ':' + encData
		...
		let DataSharingAbstractBlock_NewVersion_res = await exdr.myContract.methods.DataSharingAbstractBlock_NewVersion(
			serialNumber,
			signature,
			hashContent,
			requestor).send({
		...
		console.log('DataSharingAbstractBlock_NewVersion_res', DataSharingAbstractBlock_NewVersion_res)
	} catch (e) {
		console.log('error:', e)
	}
}
sendDataUpdateReq_recordAbsBlock(DSCJSON)